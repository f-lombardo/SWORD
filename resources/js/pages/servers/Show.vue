    ...
    <div class="mx-auto flex h-full w-full max-w-4xl flex-1 flex-col gap-6 p-6">
      ...
      <div class="flex items-start justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
          <Badge :variant="statusVariant(server.status)" class="mt-1">
            ...
            <Loader2 v-else-if="isProvisioning" class="size-3 animate-spin" />
          ...
          <Dialog v-model:open="showDeleteDialog">
            ...
            <DialogContent>
              ...
              <DialogFooter>
                ...
                <Button variant="destructive" :disabled="deleteForm.processing" @click="deleteServer">
      ...
      <template v-if="isPending || isProvisioning">
        ...
        <div class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border">
          <div class="flex items-center gap-3 border-b border-sidebar-border/70 px-5 py-4 dark:border-sidebar-border">
            <Terminal class="size-5 shrink-0 text-muted-foreground" />
            <div>
          ...
          <div class="flex flex-col gap-3 p-5">
            <div
              ...
              <pre
                class="overflow-x-auto p-4 font-mono text-sm leading-relaxed break-all whitespace-pre-wrap text-foreground">{{ server.wget_command }}</pre>
              <Button variant="ghost" size="icon"
                class="absolute top-2 right-2 size-7 opacity-0 transition-opacity group-hover:opacity-100"
                @click="copyCommand" :title="copied ? 'Copied!' : 'Copy command'">
        ...
        <div class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border">
          <div
            ...
            <div class="flex items-center gap-3">
              <Activity class="size-5 shrink-0 text-muted-foreground" />
              <p class="text-sm font-medium">
                Provisioning progress
              </p>
            </div>
            <div class="flex items-center gap-2">
              <Loader2 v-if="isProvisioning" class="size-3.5 animate-spin text-muted-foreground" />
              <span class="text-xs text-muted-foreground tabular-nums">{{ progressPercent }}%</span>
            </div>
          </div>
          ...
          <!-- Step list -->
          <div class="max-h-96 divide-y divide-sidebar-border/50 overflow-y-auto dark:divide-sidebar-border/30">
            <div v-for="(key, index) in STEP_KEYS" :key="key" class="flex items-center gap-3 px-5 py-2.5 text-sm"
              :class="{
                'opacity-40':
                  !isStepCompleted(index) &&
                  !isStepActive(index),
              }">
              <CheckCircle2 v-if="isStepCompleted(index)" class="size-4 shrink-0 text-green-500" />
              <Loader2 v-else-if="isStepActive(index)" class="size-4 shrink-0 animate-spin text-primary" />
              <div v-else class="size-4 shrink-0 rounded-full border-2 border-muted-foreground/30" />
              <span :class="{
                'font-medium': isStepActive(index),
              }">
                {{ STEP_LABELS[key] }}
              </span>
              <span v-if="stepTimestamp(key)" class="ml-auto text-xs text-muted-foreground tabular-nums">
                {{ stepTimestamp(key) }}
              </span>
            </div>
          </div>
      ...
      <!-- Failed state -->
      <div v-if="isFailed" class="flex items-start gap-3 rounded-xl border border-destructive/50 bg-destructive/5 p-5">
        ...
        <div>
          ...
          <p class="mt-1 text-sm text-muted-foreground">
            ...
            server logs (<code class="text-xs">sword-provision.log</code>) for details, then try provisioning again.
          </p>
        </div>
      </div>
      ...
      <template v-if="isProvisioned">
        <div class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border">
          ...
          <div class="max-h-96 divide-y divide-sidebar-border/50 overflow-y-auto dark:divide-sidebar-border/30">
            <div v-for="entry in server.provision_log.filter(
              (l) => l.step !== 'started',
            )" :key="entry.timestamp + entry.step" class="flex items-center gap-3 px-5 py-2.5 text-sm">
        ...
        <div class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border">
          <div class="flex items-center gap-3 border-b border-sidebar-border/70 px-5 py-4 dark:border-sidebar-border">
            ...
            <span class="ml-auto">
              <Button size="sm" variant="outline" @click="handleAddSchedule">
                <Plus class="size-3.5" />
                Add Schedule
              </Button>
            </span>
          ...
          <div v-else class="divide-y divide-sidebar-border/50 dark:divide-sidebar-border/30">
            <div v-for="schedule in backupSchedules" :key="schedule.id"
      ...
      <Dialog v-model:open="showAddScheduleModal">
        <DialogContent class="sm:max-w-md">
          ...
          <form @submit.prevent="submitAddSchedule" class="flex flex-col gap-4 py-2">
            <div class="flex flex-col gap-1.5">
              <label class="text-sm font-medium" for="schedule-dest">Backup Destination</label>
              <select id="schedule-dest" v-model="scheduleForm.backup_destination_id"
                ...
                <option v-for="dest in backupDestinations" :key="dest.id" :value="dest.id">
                  {{ dest.name }}
                </option>
              </select>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="flex flex-col gap-1.5">
                ...
                <select id="schedule-frequency" v-model="scheduleForm.frequency" :disabled="scheduleForm.processing"
                  class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs transition-[color,box-shadow] focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <Input id="schedule-time" v-model="scheduleForm.time" type="time" :disabled="scheduleForm.processing" />
            ...
            <div v-if="scheduleForm.frequency === 'weekly'" class="flex flex-col gap-1.5">
              ...
              <select id="schedule-dow" v-model.number="scheduleForm.day_of_week" :disabled="scheduleForm.processing"
                class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs transition-[color,box-shadow] focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50">
            ...
            <div v-if="scheduleForm.frequency === 'monthly'" class="flex flex-col gap-1.5">
              ...
              <Input id="schedule-dom" v-model.number="scheduleForm.day_of_month" type="number" min="1" max="28"
                :disabled="scheduleForm.processing" />
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <Input id="schedule-retention" v-model.number="scheduleForm.retention_count" type="number" min="1"
                max="365" :disabled="scheduleForm.processing" />
              <InputError :message="scheduleForm.errors.retention_count" />
            </div>

            <DialogFooter class="pt-2">
              <Button type="button" variant="outline" @click="showAddScheduleModal = false"
                :disabled="scheduleForm.processing">
                Cancel
              </Button>
      ...
      <Dialog v-model:open="showNoDestinationsDialog">
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              You need to create a backup destination before you can schedule backups. Backup destinations define where
              your
              backups are stored.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog v-model:open="showDeleteScheduleDialog">
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" :disabled="deleteScheduleForm.processing" @click="deleteSchedule">
